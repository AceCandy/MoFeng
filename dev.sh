...
(
  cd "$FRONTEND_DIR"
  # 关闭 Vite Host 校验：远程/workspace 端口转发（如 cmux）访问时 Host 不在白名单会被 403
  exec env NODE_OPTIONS="$FRONTEND_NODE_OPTIONS" BACKEND_PROXY_HOST="$BACKEND_PROXY_HOST" BACKEND_PORT="$BACKEND_PORT" FRONTEND_HOST="$FRONTEND_HOST" FRONTEND_PORT="$FRONTEND_PORT" FRONTEND_HMR_HOST="$FRONTEND_HMR_HOST" FRONTEND_ALLOWED_HOSTS="${FRONTEND_ALLOWED_HOSTS:-true}" npm run dev
) &
FRONTEND_PID=$!
