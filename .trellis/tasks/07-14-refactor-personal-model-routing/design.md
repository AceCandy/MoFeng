...
| 11 ✅ | `SelectedModelChips.vue` + `PrimaryModelPanel.vue` 子组件 + scoped 迁移（**PrimaryModelPanel**：llm 主模型面板 L80-102 迁子，props enabledChatModels/primaryChatModel/providerName + emit set-primary 直绑 setPrimaryChatModelById[读 event.target.value]，独占 panel/primary-panel/primary-copy h3/primary-field/@media860 迁子 + `.panel` orphan 随迁[父已 0 消费]+共享 .hint 复制子；**SelectedModelChips**：已选模型 chip 列表 L324-361 迁子，props chips/activeSection + emit delete(modelName) 父 inline arrow 绑 deleteModelForActiveSection(provider,$event)，独占 selected-models/model-list-title/selected-chip-list/selected-chip[**合并 mixed 的 gap + standalone 定义**]/:hover/>span.chip-name/stamp-label/delete-btn/::before/:hover .delete-btn/delete-btn:hover/delete-btn:focus-visible 迁子 + 共享 .empty 复制子；3 处混合选择器拆分[model-list/selected-models grid→留 model-list bare pre-existing dead|picker-head/picker-row/selected-chip flex→留 picker 两类|focus-visible→留 link/toggle/provider-delete/picker-row]；父独占 class rg 零残留；CSS 大括号 85=85 配平；零 spec 断言零指针跟随） | 子组件 | 低 | 1395 → **1197** |
| 12 ✅ | `ModelPickerDialog.vue` 子组件 + Teleport/scoped/a11y 迁移（template L174-299 Teleport 弹窗整块迁子，根 `<Teleport to="body">`；15 props[provider/activeSection/modelPickerStyle/isSavingPicker/isChatPickerDirty/modelPickerQuery/pendingChatModelNames/pendingTTSModelName + 5 读函数 filteredModelNamesForProvider/isModelSelectedForActiveSection/activeModelStateLabel/savedModelForActiveSection/providerFetchState + 2 ref setter]+6 emit[update-query/toggle-chat/select-embedding/select-tts/save/close]；modelPickerQuery v-model→:value+@input emit 规避 vue/no-mutating-props[同 S9/S10]；**保留** Teleport to body/fixed 定位/`id="model-picker-${id}"` 选择器 hack/函数 ref[data-dialog-initial-focus a11y]/esc+click.stop；监听归属不变[onPickerClickOutside/onPickerViewportChange 仍在 useModelPicker document/window 级靠 id 查 DOM]；v-if=isModelPickerOpen 提到父 `<ModelPickerDialog v-if>` 整体挂卸[避空 Teleport 节点，ref 行为等价]；**顺手清 Slice 8 回归** embedding radio :checked 的 embeddingModelForName 引用[移进 composable 内部未同步 template→运行时未定义]→savedModelForActiveSection[其 embedding 分支恒等价，零 composable 改动]；scoped 独占规则全迁子[model-picker/picker-head×2/picker-search/picker-list/picker-row×4/picker-model-name×3/.link 独占]+2 处混合选择器拆分[model-controls input,.picker-row input→留 model-controls 既有死代码|link/toggle/provider-delete/picker-row focus→留 toggle/provider-delete]+@media640 model-picker 迁子+共享 .hint/.empty 复制子；父 picker 独占 class rg 零残留；CSS 大括号 父 72=72 子 17=17；spec 指针跟随 2 处[uiAuditRegression aria-label→modelPicker 源码+ttsSettings 守卫→pickerDialog 源码，3 处 emit 模式变化字符串更新：save click→@save 绑定/tts change→@select-tts 绑定/esc→emit('close')，父级事件接线断言保留读 routing]） | 子组件 | 高 | 1197 → **996** |
| 13 ✅ | `ProviderCard.vue` 子组件 + scoped 迁移（template L88-213 provider-card 整块 article 迁子[行内 edit 表单 ProviderFormPanel mode=edit + 常态展示 header[state/type/key/url + toggle/delete actions] + provider-actions[编辑/拉取] + hints + SelectedModelChips]；**方案 B：ModelPickerDialog 留父 v-for 兄弟节点**[picker composable 单例必须父实例化，避方案 A 的 20 props 双层透传，picker 接线 24 行留父 spec 断言字符串原样命中]；8 props[provider/isEditing/activeSection/providerForm/isSavingProvider/providerFetchState/isModelPickerOpen/selectedModelChipsForProvider]+8 emit[update-field/save-provider/cancel-provider/toggle/delete/edit/open-picker/delete-model]；providerTypeLabel 子直 import modelRoutingHelpers[纯函数]+providerKeyLabel 子内定义[迁子]；删父 3 orphan[providerKeyLabel 函数+UserModelProvider import+providerTypeLabel import]；scoped 跨组件同 S3/S9：卡片内部独占规则全迁子[provider-head/provider-main/title-row/meta/state/type/url/card-actions/toggle/provider-delete/provider-actions/md-btn-tonal/md-btn-text/hint.is-error]+卡片根 provider-card/::after/is-editing 留父靠子根继承父 data-v；4 处共享 selector 拆分[S1 topbar/provider-head/model-row flex->留 topbar/model-row|S2 provider-head p/hint/empty->留 hint[顶栏 L7 section-copy 仍用]+empty|S5 feedback.is-error/hint.is-error->留 feedback.is-error|S7 @media640 topbar/provider-head/provider-actions/model-row->留 topbar/model-row]+2 处整块迁子[toggle:disabled/provider-delete:disabled + toggle/provider-delete:focus-visible]；@media768 provider-grid/card:only-child/is-editing 留父[card 根]；pre-existing dead[model-list/model-row/model-controls]保守留父；父卡片内部独占 class rg 零残留；CSS 大括号 父 41=41 子 35=35；零 spec 断言零指针跟随[picker 接线留父断言原样命中]） | 子组件 | 中 | 996 -> **616** |
| 14 ✅ | dead CSS 清理[model-list/model-row/model-controls pre-existing 用户批准删除]+2 处共享 selector 拆分留 topbar[L360/L578]；ReadinessPanel.vue 抽子组件[props summary:ReadinessSummary；template L48-56 readiness 块迁子 label/value/description/tone；style 6 块全迁子 readiness 根+is-success/is-warning 后代+label/value/detail；scoped 子内部后代选择器命中非跨组件；父 `<ReadinessPanel :summary>` + import L157]；rg 零残留 | style+子组件 | 中 | 616 -> **514** |
| 15 ✅ | FeedbackPanel 子组件抽[props feedback:FeedbackMessage 内联同型 interface；template L39-46 feedback 块迁子 v-if 守卫留父；style 3 块全迁 feedback+is-success/is-error 单元素选择器子根带 data-v-child 命中]；选 feedback 不选 empty-state[feedback 纯展示无交互无 emit，empty-state 有 button 需 emit create+透传 activeSection 偏离纯展示]；rg 零残留 | 子组件 | 中 | 514 -> **493 ✅<500** |
