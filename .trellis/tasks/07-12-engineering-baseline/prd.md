...
- 🔄 **Slice D** WDWorkspace template 子组件抽取（2026-07-13 起，Slice B 抽完 composable 后的延续）：✅ 第 1 块 `EditChapterModal`——composable useEditChapterModal 随 template 迁入子组件（useDialogA11y 子 setup 调用，父按钮 ref 调 openEditModal），WDWorkspace 1980→1844（−136），vue-tsc/vitest 120 绿 / eslint 0 新增；⏳ 后续 3 块（ChapterEvaluationPanel ~251 行 / ChapterVersionsPanel ~112 行 / WorkspaceHeader ~500 行）按 `design.md` Slice D 契约表风险递增跨会话推进。

> Slice B/C/D 未完成，Acceptance 第 4 项前半句「5 大组件 <500 行」仍需跨会话达成（当前前 5 大：PersonalModelRouting 2684 / ChapterGenerating 2261 / WritingDesk 2009 / WDWorkspace 1844 / NovelDetailShell 1662）。
