...
_NO_RESUME = object()
_WAIT_PROGRESS = {
    "waiting_for_selection": 60,
    "projection_pending": 90,
}


class ChapterWorkflowRuntime:
    """在短生命周期 saver 连接内执行或恢复一次 Graph V1。"""
        ...
        *,
        ...
        async with open_chapter_workflow_checkpointer(self.database_url) as saver:
            ...
            if command_handshake and snapshot.values:
                state = ChapterWorkflowStateV1.model_validate(snapshot.values)
            ...
            if marker_already_written:
                ...
                waiting = self._waiting_outcome(snapshot)
                if waiting is not None:
                    return waiting
                graph_input: Any = (
                    None
                    if snapshot.values
                    else ChapterWorkflowStateV1.initial(
    ...
    def _payload(self) -> ChapterWorkflowJobPayload:
        ...
        return ChapterWorkflowJobPayload.model_validate(lease.payload)

    @staticmethod
    def _waiting_outcome(snapshot: Any) -> JobWaitOutcome | None:
    ...
    def _validate_command_resume(
        *,
        ...
        if node_key == "waiting_for_selection":
            ...
            return
        if node_key == "projection_pending" and set(resume_value) == {"command_id"}:
            return
