# AIMETA P=章节状态事件总线_Redis pubsub 轻量通知|R=publish通知_subscribe channel|NR=不含业务逻辑查询|E=event_bus|X=internal|A=服务|D=redis|S=cache|RD=./README.ai
"""章节状态事件总线：基于 Redis pub-sub 的轻量通知。

publish 端在生成流程状态变更后发出通知（只发 channel 信号，不发业务数据）；
SSE 端 subscribe channel，收到通知后再查 DB 推送完整快照。
Redis 不可用或未配置时 publish 静默跳过（不阻塞生成流程），subscribe 返回 None 由调用方回退轮询。
"""
...
def get_async_client() -> Optional[aioredis.Redis]:
    ...
    if not settings.redis_url:
        logger.info("未配置 REDIS_URL，章节状态事件总线禁用")
        return _async_client
    try:
        ...
        logger.info("章节状态事件总线 Redis 客户端已初始化")
    except Exception as e:
        logger.warning(f"章节状态事件总线 Redis 客户端初始化失败: {e}")
        _async_client = None
    return _async_client
...
async def _safe_publish(client: aioredis.Redis, channel: str) -> None:
    ...
    try:
        ...
        logger.warning(f"发布章节状态通知失败 channel={channel}: {e}")
