...
class ImportService:
    ...
    async def import_novel_from_file(self, user_id: int, file: UploadFile) -> str:
        ...
        # 预提取人名 (基于全文)
        potential_characters = self._extract_potential_characters(
            content, top_n=150
        )  # 扩大到150，广撒网
        ...
        char_highlights_text = self._extract_character_highlights(
            content, potential_characters, context_window=200
        )

        # 3. 分阶段分析
        # 阶段一：先筛选出确定的角色名单 (Stable Census)
        verified_characters = await self._filter_characters_only(
            user_id, potential_characters, char_highlights_text
        )
        logger.info(
            f"角色筛选完成，潜在 {len(potential_characters)} -> 确认 {len(verified_characters)}"
        )
        ...
        if blueprint_data.chapter_outline:
            ...
            for i, (chap_title, _) in enumerate(chapters, 1):
                if i in ai_outlines:
                    ...
                    outline = ChapterOutlineSchema(chapter_number=i, title=chap_title, summary="")
        ...
        for i, (chap_title, chap_content) in enumerate(chapters, 1):
            ...
            await self.novel_service.replace_chapter_versions(
                chapter, [chap_content], metadata=[{"source": "file_import"}]
            )
            # 自动选择第一个版本（即导入的内容）
    ...
    def _extract_character_highlights(
        self, content: str, characters: List[str], context_window: int = 300
    ) -> str:
        """
        为每个潜在角色提取一段“高光时刻”。
        优先选择对话密集或有动作描写的段落。
        ...
        for char in characters:
            ...
            if total <= 10:
                ...
                sample_indices = (
                    list(range(3))
                    + list(range(total // 2 - 1, total // 2 + 2))
                    + list(range(total - 3, total))
                )
    ...
    async def _filter_characters_only(
        self, user_id: int, potential_characters: List[str], char_highlights: str
    ) -> List[str]:
        """
        阶段一：角色普查。
        ...
        if not system_prompt:
            raise HTTPException(
                status_code=500,
                detail="缺少导入角色鉴别提示词，请联系管理员配置 'import_character_filter'",
            )
    ...
    async def _analyze_content(
        ...
        if not prompt_template:
            raise HTTPException(
                status_code=500, detail="缺少导入分析提示词，请联系管理员配置 'import_analysis'"
            )
