# AIMETA P=业务任务注册_生产worker路由|R=注册版本化业务handler_注入workflow依赖|NR=不执行worker循环|E=build_job_handler_registry|X=internal|A=composition_root|D=job_registry|S=memory|RD=./README.ai
...
def register_job_handlers(
    ...
    *,
    ...
    registry.register(
        ...
        payload_version=1,
        side_effect_class=SideEffectClass.AMBIGUOUS_EXTERNAL,
        handler=build_chapter_workflow_job_handler(
            database_url,
            provider_factory=chapter_workflow_provider_factory,
        ),
    )
    registry.register(
...
def build_job_handler_registry(
    *,
    ...
    return register_job_handlers(
        JobHandlerRegistry(),
        database_url=database_url or settings.sqlalchemy_database_uri,
        chapter_workflow_provider_factory=chapter_workflow_provider_factory,
    )
