...
class FactionService:
    ...
    async def get_faction(self, faction_id: int) -> Optional[Faction]:
        """获取势力"""
        result = await self.db.execute(select(Faction).where(Faction.id == faction_id))
        return result.scalar_one_or_none()

    async def get_factions_by_project(self, project_id: str) -> List[Faction]:
        """获取项目的所有势力"""
        result = await self.db.execute(select(Faction).where(Faction.project_id == project_id))
    ...
    async def get_faction_writing_context(
        self, project_id: str, involved_factions: Optional[List[int]] = None
    ) -> str:
        """获取写作用的势力上下文（可指定相关势力）"""
    ...
    def _build_relationships_overview(
        self, factions: List[Faction], relationships: List[FactionRelationship]
    ) -> str:
        """构建关系概览"""
    ...
    async def check_faction_consistency(
        self, project_id: str, chapter_content: str
    ) -> Dict[str, Any]:
        """检查章节中势力描写的一致性"""
        factions = await self.get_factions_by_project(project_id)
