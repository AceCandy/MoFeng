...
class EnhancedWritingFlow:
    ...
    def __init__(self, db: AsyncSession, llm_service: LLMService, prompt_service: PromptService):
        ...
        self.review_service = SixDimensionReviewService(
            db, llm_service, prompt_service, self.constitution_service, self.writer_persona_service
        )
    ...
    def build_enhanced_prompt_sections(
        self, base_sections: List[tuple], enhanced_context: Dict[str, Any]
    ) -> List[tuple]:
        """
        构建增强的提示词段落
        """
        ...
        # 添加势力关系
        if (
            enhanced_context.get("faction_context")
            and enhanced_context["faction_context"] != "（无势力设定）"
        ):
    ...
    def _format_foreshadowing_reminders(self, reminders: Dict[str, Any]) -> str:
        ...
        for fs in reminders.get("foreshadowings_to_develop", []):
            urgency_emoji = (
                "🔴"
                if fs.get("urgency") == "high"
                else ("🟡" if fs.get("urgency") == "medium" else "🟢")
            )
    ...
    async def update_foreshadowing_status_from_content(
        ...
        # 获取活跃伏笔
        active_foreshadowings = await self.foreshadowing_service.get_active_foreshadowings(
            project_id
        )
    ...
    async def get_writing_health_report(self, project_id: str) -> Dict[str, Any]:
        ...
        # 检查伏笔健康度
        foreshadowing_health = await self.foreshadowing_service.analyze_foreshadowing_health(
            project_id
        )
