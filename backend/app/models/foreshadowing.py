...
class Foreshadowing(Base):
    ...
    chapter_revision: Mapped[int] = mapped_column(
    ...
    )  # question, mystery, hint, clue, setup
    keywords: Mapped[Optional[list]] = mapped_column(JSON, default=list)

    # 伏笔状态
    status: Mapped[str] = mapped_column(
        String(32), default="planted", index=True
    )  # planted, developing, revealed, abandoned, partial
    ...
    resolved_chapter: Mapped[Optional["Chapter"]] = relationship(
        "Chapter", foreign_keys=[resolved_chapter_id]
    )
...
class ForeshadowingResolution(Base):
    ...
    id: Mapped[int] = mapped_column(BIGINT_PK_TYPE, primary_key=True, autoincrement=True)
    foreshadowing_id: Mapped[int] = mapped_column(
        ForeignKey("foreshadowings.id", ondelete="CASCADE"), nullable=False
    )

    # 回收信息
    resolved_at_chapter_id: Mapped[Optional[int]] = mapped_column(
    ...
    resolved_chapter: Mapped["Chapter"] = relationship(
        "Chapter", foreign_keys=[resolved_at_chapter_id]
    )
...
class ForeshadowingReminder(Base):
    ...
    reminder_type: Mapped[str] = mapped_column(
        String(32), nullable=False
    )  # unresolved, long_time_no_mention, pattern_mismatch
    message: Mapped[str] = mapped_column(LONG_TEXT_TYPE, nullable=False)
    suggested_chapter_range: Mapped[Optional[dict]] = mapped_column(
        JSON
    )  # {"start": 10, "end": 20}
