...
class NovelConstitution(Base):
    ...
    # ===== 叙事视角 =====
    pov_type: Mapped[Optional[str]] = mapped_column(
        String(64)
    )  # POV 类型（第一人称/第三人称有限/全知等）
    ...
    def to_prompt_context(self) -> str:
        ...
        # 目标受众
        if any(
            [self.target_age_group, self.reading_level, self.violence_rating, self.romance_rating]
        ):
        ...
        if any(
            [
                self.allowed_character_types,
                self.character_power_limits,
