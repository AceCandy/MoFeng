...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-2xl font-bold text-[var(--md-on-surface)]">主要角色</h2>
        <p class="text-sm text-[var(--md-on-surface-variant)]">了解故事中核心人物的目标与个性</p>
      </div>
      <button
        v-if="editable"
        type="button"
      ...
      <article
        ...
        <div class="p-6">
          <div class="flex flex-col sm:flex-row sm:items-center gap-4 mb-4">
            ...
            <div>
              <h3 class="text-xl font-bold text-[var(--md-on-surface)]">{{ character.name || '未命名角色' }}</h3>
              <p v-if="character.identity" class="text-sm text-[var(--md-primary)] font-medium">{{ character.identity }}</p>
            </div>
          </div>
          <dl class="space-y-3 text-sm text-[var(--md-on-surface-variant)]">
            <div v-if="character.personality">
              <dt class="font-semibold text-[var(--md-on-surface)] mb-1">性格</dt>
              <dd class="leading-6">{{ character.personality }}</dd>
            </div>
            <div v-if="character.goals">
              <dt class="font-semibold text-[var(--md-on-surface)] mb-1">目标</dt>
              <dd class="leading-6">{{ character.goals }}</dd>
            </div>
            <div v-if="character.abilities">
              <dt class="font-semibold text-[var(--md-on-surface)] mb-1">能力</dt>
              <dd class="leading-6">{{ character.abilities }}</dd>
            </div>
            <div v-if="character.relationship_to_protagonist">
              <dt class="font-semibold text-[var(--md-on-surface)] mb-1">与主角的关系</dt>
              <dd class="leading-6">{{ character.relationship_to_protagonist }}</dd>
            </div>
          </dl>
        </div>
      </article>
      <div v-if="!characters.length" class="bg-[var(--md-surface)] rounded-2xl border border-dashed border-[var(--md-outline-variant)] p-10 text-center text-[var(--md-on-surface-variant)]">
        暂无角色信息
      </div>
    </div>
  </div>
