...
<template>
  <div class="space-y-4 max-h-96 overflow-y-auto p-1">
    <div v-for="(faction, index) in localFactions" :key="index" class="p-4 border border-[var(--md-outline-variant)] rounded bg-[var(--md-surface-container-low)] relative">
      <button
        type="button"
    ...
    <div v-if="localFactions.length === 0" class="blueprint-empty">
      <div>
        <p class="blueprint-empty__title">暂无势力</p>
        <p class="blueprint-empty__desc">点击下方按钮添加第一个势力</p>
      </div>
    </div>
...
const emit = defineEmits<{
  'update:modelValue': [value: Faction[]]
}>()
