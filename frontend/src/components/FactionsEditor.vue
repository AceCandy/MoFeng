...
<template>
  <div class="space-y-4 max-h-96 overflow-y-auto p-1">
    ...
    <button type="button" @click="addFaction" class="w-full mt-4 px-4 py-2 text-sm font-medium text-[var(--md-primary)] bg-[var(--md-primary-container)] border border-[var(--md-primary-container)] rounded-md hover:bg-[var(--md-primary-dark)] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--md-primary-light)]">
      + 添加新阵营
    </button>
  </div>
</template>
